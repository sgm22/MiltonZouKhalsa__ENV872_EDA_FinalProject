...
kable(collapsed_dat_settlement_type, caption = "Summary of COVID and Population Data by Settlement Types") #Generate Table
...
shapiro.test(log(combined_dat_ny_covid_pop$sum_cumulative_no_of_positive)) #Check normality
...
shapiro.test(log(combined_dat_ny_covid_pop$sum_cumulative_no_of_tests)) #Check normality
...
shapiro.test(log(combined_dat_ny_covid_pop$density_permilessquared)) #Check normality
...
### Spatial Context of Data (does not work)
...
## Summary inference: Residual deviance is much larger than degrees of freedom (DF). Negative binomial plot better.
...
change, the the ranks of the number of COVID-19 testsconducted do not change.
...
>After determining that the correlation between absolute population and number 
of COVID-19 tests conducted is stronger, we examined within those parameters to
see if this correlation varies among settlement types. We performed the Spearman
rank correlation test on all three settlement types. Urban settlements had a
p-value of 2.2e-16 and an R-squared value of 0.96, micropolitan settlements had 
a p-value of 0.002 and an R-squared value of 0.77, and rural settlements had a
p-value of 1 and an R-squared value of 0.5. We reject the null hypotheses to 
accept the alternative hypotheses for urban and micropolitan settlements, but 
accept the null hypothesis for rural settlements. The correlation is strongest 
with urban settlements. However, we do acknowledge that the rural settlements 
only had 3 data points, which is much fewer than the amount of data of micropolitan 
...
the number of COVID-19 tests conducted in New York. In both cases, the absolute
population correlation was the stronger of the two. There is also a strong
correlation between absolute population and urban settlements.
