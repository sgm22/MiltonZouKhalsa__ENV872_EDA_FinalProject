...
2. What is the relationship between the predictor variables: density, absolute population,and settlement types and the response variable: cumulative COVID-19 cases?

3. What is the correlation between absolute population and population density 
and the number of COVID-19 tests does done? Does this the relationship with the
stronger correlated relationship vary among settlement types? 
...
## Question 2:  What is the relationship between the predictor variables: density, absolute population,and settlement types and the response variable: cumulative COVID-19 cases?

### Poisson Regression: Absolute Population and COVID-19 Cases
...
3. The distribution of counts follow a Poisson distribution.
...
summary (modelpoisson <- glm(log(sum_cumulative_no_of_positive) ~ log(density_permilessquared) + log(pop2022) + factor(settlement_type), family = quasipoisson, data = combined_dat_ny_covid_pop))
...
```{r}
# Read in New York counties shapefile into an sf dataframe, filtering for just New York counties.
